...
public class Fractal {
	...
	public int bsETA(double xInput, double yInput){
		xCalc = xInput;
		yCalc = yInput;
		double newX;
		double newY;
	...
	public int jETA(double xInput, double yInput){
		xCalc = xInput;
		yCalc = yInput;
		double dist = Math.sqrt(xCalc * xCalc + (yCalc * yCalc));
		int passes = 0;
	...
	public int maETA(double xInput, double yInput){
		xCalc = xInput;
		yCalc = yInput;
		double dist = Math.sqrt((xCalc*xCalc)+(yCalc*yCalc));
		int passes = 0;
	...
	public int muETA(double xInput, double yInput){
		xCalc = xInput;
		yCalc = yInput;
		double dist = Math.sqrt((xCalc*xCalc)+(yCalc*yCalc));
		int passes = 0;
	...
	public int[][] bsArray(){
		for(int i = 0; i < bsFractal.length; i++){
			for(int j = 0; j<bsFractal[0].length; j++){
				bsFractal[i][j] = bsETA(bsTranslateX(i), bsTranslateY(j));
				}
			}
	...
	public int[][] jArray(){
		for(int i = 0; i < jFractal.length; i++){
			for(int j = 0; j<jFractal[0].length; j++){
				jFractal[i][j] = jETA(jTranslateX(i), jTranslateY(j));
				}
			}
	...
	public int[][] maArray(){
		for(int i = 0; i < maFractal.length; i++){
			for(int j = 0; j<maFractal[0].length; j++){
				maFractal[i][j] = maETA(maTranslateX(i), maTranslateY(j));
				}
			}
	...
	public int[][] muArray(){
		for(int i = 0; i < muFractal.length; i++){
			for(int j = 0; j<muFractal[0].length; j++){
				muFractal[i][j] = muETA(muTranslateX(i), muTranslateY(j));
				}
			}
